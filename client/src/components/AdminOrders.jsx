...
const AdminOrders = () => {
  ...
  const user = isAdmin ? adminUser : authUser;

  const { userOrders, adminOrders, loading, error } = useSelector(
    (state) => state.order
  );

  const ordersToShow = isAdmin ? adminOrders : userOrders;
  ...
  return (
    <div className="md:p-10 p-4 space-y-4 dark:bg-[var(--neutral-50)]">
      <h2 className="text-lg font-medium dark:text-[var(--neutral-700)]">
        {isAdmin ? "All Orders (Admin View)" : "Your Orders"}
      </h2>

      {ordersToShow.map((order) => (
        <div
          key={order._id}
          className="flex flex-col md:grid md:grid-cols-[2fr_1.2fr_1fr_1fr_1fr] md:items-center gap-5 p-5 max-w-6xl rounded-md border border-gray-300 text-gray-800 dark:border-[var(--neutral-200)] dark:bg-[var(--neutral-50)] dark:text-[var(--neutral-700)]"
        >
          {/* Products */}
